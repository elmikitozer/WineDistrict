...
type UserRow = { id: string | number; email: string; role: string; passwordHash: string };
...
export async function POST(req: Request) {
  ...
  const user = (await (
    prisma as unknown as { user?: { findFirst: (args: unknown) => Promise<UserRow | null> } }
  ).user?.findFirst({
    where: { email },
    include: { caviste: true },
  })) as (UserRow & { caviste?: unknown }) | null;
